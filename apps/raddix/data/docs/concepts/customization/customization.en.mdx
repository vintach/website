...
Each hook has a default <code>as</code> prop. This is
important since according to the selected <code>as</code>
Raddix will provide the necessary accessibility.

<br />
For example, the useSwitch hook assumes that it will be represented on an
HTML <code>{'<div>'}</code>, to be able to represent it on a different DOM
tag as a <code>{'<button>'}</code> should change the <code>as</code> prop.
